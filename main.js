...
if (custom[4] != null) {
    fetch("https://api.github.com/repos/" + custom[3] + "/" + custom[4] + "/languages").then(r => r.json()).then((obj) => {
        ...
        for (i = 0; i < keys.length; i++) {
            text += '<a class="d-inline-flex flex-items-center flex-nowrap link-gray no-underline text-small mr-3"><span class="text-gray-dark text-bold mr-1">' + keys[i] + '</span><span>' + (obj[Object.keys(obj)[i]] / sum * 100).toFixed(1) + '%</span></a>';
        }
