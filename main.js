...
if (custom[4] != null) {
    fetch("https://raw.githubusercontent.com/ozh/github-colors/master/colors.json").then(r => r.json()).then((color) => {console.log(color)
    fetch("https://api.github.com/repos/" + custom[3] + "/" + custom[4] + "/languages").then(r => r.json()).then((obj) => {
        ...
        for (i = 0; i < keys.length; i++) {
            text += '<a class="d-inline-flex flex-items-center flex-nowrap link-gray no-underline text-small mr-3"><svg class="octicon octicon-dot-fill mr-2" style="color:'+color[ keys[i] ].color+';" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path fill-rule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8z"></path></svg><span class="text-gray-dark text-bold mr-1">' + keys[i] + '</span><span>' + (obj[Object.keys(obj)[i]] / sum * 100).toFixed(1) + '%</span></a>';
        }
